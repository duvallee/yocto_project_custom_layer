...
do_configure_extra() {
   bbplain "------------------"
   bbplain "OECMAKE         : ${EXTRA_OECMAKE}"
   bbplain "PACKAGECONFIG   : ${PACKAGECONFIG}"
   bbplain "TARGET_CC_ARCH  : ${TARGET_CC_ARCH}"
   bbplain "------------------"
}
addtask configure_extra before do_configure
...
# ----------------------------------------------------------------------
EXTRA_OECMAKE = " \
    -D OPENCV_EXTRA_MODULES_PATH=${WORKDIR}/contrib/modules \
    -D WITH_1394=OFF \
    -D ENABLE_PRECOMPILED_HEADERS=OFF \
    -D CMAKE_SKIP_RPATH=ON \
    -D OPENCV_ICV_HASH=${IPP_MD5} \
    -D IPPROOT=${WORKDIR}/ippicv_lnx \
    -D OPENCV_GENERATE_PKGCONFIG=ON \
    -D OPENCV_DOWNLOAD_PATH=${OPENCV_DLDIR} \
    -D OPENCV_ALLOW_DOWNLOADS=OFF \
...
EXTRA_OECMAKE_append_raspberrypi4 = " -D ENABLE_NEON=ON -DBUILD_opencv_dnn=ON -DPROTOBUF_UPDATE_FILES=OFF -DBUILD_PROTOBUF=OFF "
# EXTRA_OECMAKE_append_raspberrypi4-64 = " -D ENABLE_NEON=ON"

# ----------------------------------------------------------------------
PACKAGECONFIG ??= "gapi python3 eigen jpeg png tiff v4l libv4l gstreamer tbb gphoto2 \
